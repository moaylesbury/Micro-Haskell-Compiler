...
/*
static class CommentAcceptor extends Acceptor implements DFA {					//TODO: MAY BE PROBLEMATIC
	public String lexClass() {return "";}
	...
	int next (int state, char c) {
		...
		case 2: if (!CharTypes.isNewline(c) && !CharTypes.isSymbolic(c)) return 3 ; else if (c =='-') return 2; else return 4;
...
static class CommentAcceptor extends Acceptor implements DFA {					//TODO: ALTERNATE COMMENT ACCEPTOR
	public String lexClass() {return "";}
	public int numberOfStates() {return 5;}
...
static class TokAcceptor extends Acceptor implements DFA {
	...
	int next (int state, char c) {
		return (state < tokLen && c == tok.charAt(state)) ? state + 1 : tokLen + 1;
	}
	...
	static DFA[] MH_acceptors = new DFA[] {whiteAcc, commAcc, symAcc, symAcc, intAcc, ifAcc, bAcc, thenAcc, elseAcc, lbraAcc, rbraAcc, scAcc, boolAcc, numAcc, varAcc};
